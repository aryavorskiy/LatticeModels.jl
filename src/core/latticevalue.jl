...
site_inds(l::LT, lv_mask::LatticeValue{Bool, LT}) where LT<:AbstractLattice =
    Int[site_index(l, lattice(lv_mask)[i])
        for i in eachindex(lv_mask.values) if lv_mask.values[i]]
Base.@propagate_inbounds function Base.getindex(l::LT, lv_mask::LatticeValue{Bool, LT}) where LT<:AbstractLattice
